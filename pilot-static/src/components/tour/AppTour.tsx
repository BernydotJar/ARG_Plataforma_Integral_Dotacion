...
export function AppTour({ pathname, userKey, onClose }: AppTourProps) {
  ...
  useEffect(() => {
    ...
    const updateGeometry = () => {
      ...
      const cardTop = showCardBelow
        ? clamp(rect.bottom + 16, EDGE_GAP, viewportHeight - CARD_ESTIMATED_HEIGHT - EDGE_GAP)
        : clamp(rect.top - CARD_ESTIMATED_HEIGHT - 16, EDGE_GAP, viewportHeight - CARD_ESTIMATED_HEIGHT - EDGE_GAP);
  ...
  if (!visibleSteps.length) {
    return createPortal(
      <div className="app-tour-layer" aria-live="polite">
  ...
  const handleNext = () => {
    if (isLastStep) {
      if (typeof window !== "undefined") {
        ...
        window.localStorage.setItem(`argos:tour:${TOUR_VERSION}:${userKey || "anon"}`, JSON.stringify(payload));
      }
      onClose();
      return;
    }
