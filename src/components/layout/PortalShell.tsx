...
import { logoutFromEntra } from "@/lib/auth/msal-client";
...
type TourVariant = "original" | "glass";
...
export function PortalShell({ user, children }: PortalShellProps) {
  ...
  const [tourVariant, setTourVariant] = useState<TourVariant>("original");
  ...
  const startTour = (variant: TourVariant) => {
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      // If the user logged in via Microsoft SSO, sign them out from Entra too.
      // logoutFromEntra navigates away so no further code runs after it.
      if (user.tenantId) {
        await logoutFromEntra();
        return;
      }
    } catch {
      // Best-effort — always redirect to login
    }
        ...
        <header className="portal-topbar" data-tour="topbar">
          ...
          <div className="portal-topbar-right">
            <Menu positioning="below-end">
              ...
              <MenuPopover>
                <MenuList>
                  <MenuItem onClick={() => startTour("original")}>Tour original</MenuItem>
                  <MenuItem onClick={() => startTour("glass")}>Tour glass (sin traslape)</MenuItem>
                </MenuList>
              </MenuPopover>
            </Menu>
